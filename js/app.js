(() => {
    ...
    function menuInit() {
        const menuBody = document.querySelector(".menu__wrap");
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) new swiper_core_Swiper(".page-videos__slider", {
            ...
            pagination: {
                el: ".page-videos__swiper-pagination",
                clickable: true,
            ...
            on: {
                slideChange: function() {
                    ...
                    if (activeBullet) {
                        var container = document.querySelector(".page-videos__swiper-pagination");
                        Array.from(activeBullet.parentNode.children).indexOf(activeBullet);
                        var newPosition = (activeBullet.offsetLeft + activeBullet.offsetWidth - container.clientWidth) * 2;
                        container.scrollTo({
                            left: newPosition,
                            behavior: "smooth"
                        });
        ...
        var tvprogramSlider = document.querySelector(".tvprogram__slider");
        if (tvprogramSlider) {
            var swiper = null;
            function initSwiper() {
                return new swiper_core_Swiper(tvprogramSlider, {
                    ...
                    scrollbar: {
                        el: ".tvprogram__swiper-scrollbar",
                        draggable: true
                    },
                    breakpoints: {
                        0: {
            ...
            function initOrDestroySwiper() {
                ...
                if (screenWidth >= 991.98 && !swiper) swiper = initSwiper(); else if (screenWidth < 991.98 && swiper) destroySwiper();
            }
            initOrDestroySwiper();
    ...
    if (searchForms) searchForms.forEach((form => {
        const searchInput = form.querySelector(".search__input");
        const suggestionsList = form.querySelector(".search__suggestions-list");
        const clearInputBtn = form.querySelector(".search__clear");
        function clearInput() {
            searchInput.value = "";
        ...
        searchInput.addEventListener("input", (() => {
            ...
            const suggestions = [ "Новий Фільмовий Хіт: Що Очікувати від Ефіру на 'Ми Україна+' Цього Тижня?", "Фільмовий Ексклюзив: Переглядайте Прем'єри на Каналі 'Ми Україна+'", "Фільмовий Розклад: Які Кінострічки Завтра Покажуть на Телеканалі?", "Вечір Фільмів: Огляд Найцікавіших Стрічок на 'Ми Україна+'", "Фільмова Феєрія: Підбірка Новинок на Каналі 'Ми Україна+'", "Завжди Перші: Свіжі Фільми на Телеканалі 'Ми Україна+' Цього Тижня", "Ексклюзивне Кіно: Що Показують у Вікенд на 'Ми Україна+'?", "Фільмовий Вікенд: Найочікуваніші Стрічки для Перегляду", "Українські Кіношедеври: Відомі Фільми, Що Показують на 'Ми Україна+'", "Не Пропустіть: Ексклюзивні Фільми, Що Виходять на Телеканалі 'Ми Україна+'" ];
    ...
    var mainVideosExists = document.querySelector(".main-videos");
    if (mainVideosExists !== null) {
        window.addEventListener("resize", (function() {
            moveButton();
        }));
        function moveButton() {
            var screenWidth = window.innerWidth;
            var mainVideosSections = document.querySelectorAll(".main-videos");
            mainVideosSections.forEach((function(section) {
                var button = section.querySelector(".main-videos__link--btn_dark");
                if (screenWidth <= 767.98) {
                    var container = section.querySelector(".main-videos__container");
                    container.appendChild(button);
                } else {
                    var topDiv = section.querySelector(".main-videos__top");
    ...
    function rearrangeBlocks() {
        const tabs = document.querySelector(".tvprogram-tabs");
        if (tabs) {
            const items = document.querySelectorAll(".tvprogram-tabs__item");
            items.forEach((item => {
                const block = item.querySelector(".tvprogram-tabs__block");
                const content = item.querySelector(".program-content__wrap");
                if (window.innerWidth < 991.98) content.insertBefore(block, content.firstChild); else item.insertBefore(block, item.firstChild);
            }));
        }
    }
    window.addEventListener("load", rearrangeBlocks);
    window.addEventListener("resize", rearrangeBlocks);
    function dataShowmoreContent() {
        const tabs = document.querySelector(".tvprogram-tabs");
        if (tabs) {
            const tvprogramWraps = document.querySelectorAll(".tvprogram-tabs__wrap");
            tvprogramWraps.forEach((tvprogramWrap => {
                if (window.innerWidth < 991.98) tvprogramWrap.dataset.showmoreContent = "540"; else tvprogramWrap.dataset.showmoreContent = "493";
            }));
        }
    }
    window.addEventListener("load", dataShowmoreContent);
    window.addEventListener("resize", dataShowmoreContent);
    window["FLS"] = false;
    isWebp();
    menuInit();
    spollers();
